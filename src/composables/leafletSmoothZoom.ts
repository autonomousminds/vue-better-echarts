 ...
 * Ported from Evidence's LeafletSmoothZoom.js
 */
import type L from 'leaflet';

export function initSmoothZoom(Leaflet: typeof L): void {
  Leaflet.Map.mergeOptions({
    smoothWheelZoom: true,
    smoothSensitivity: 1
  });

  const SmoothWheelZoom = Leaflet.Handler.extend({
    addHooks(this: any) {
  ...
  (Leaflet.Map as any).SmoothWheelZoom = SmoothWheelZoom;
  Leaflet.Map.addInitHook('addHandler', 'smoothWheelZoom', SmoothWheelZoom);
}
